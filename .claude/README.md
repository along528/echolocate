...
- **`github.com` blocked** → you can't provision natively in that session. Use
  the prebuilt dev container (`vector-rs/Dockerfile.dev`) or the Managed Agents
  worker (`vector-rs/Dockerfile.worker`) as the environment's base image instead,
  since those bake every dependency at image-build time.
